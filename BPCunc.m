...
%varargin is optionally the number of cores to use for processing
    ...
    parfor i = 1:numfids
        ...
        if(size(nametest,1)==0)
            ...
            logLk = zeros(1,N*M);
            for m = 1:M*N
                [pchain, logLk(m)] = chain_const_noMCMC(N_coefs, [samples(:,m) zeros(n,1)], lxmin, lxmax, p_sig, delta, N_pts);
            end
    ...
    parfor l = 1:k
        ...
        if(size(nametest,1)==0)
            ...
            for m = 1:N*M
                ...
                [jointchain, jlogLk(m)] = chain_const_noMCMC(N_coefs,[resamples1(:,m) zeros(n1,1);resamples2(:,m) zeros(n2,1)],lxmin,lxmax,p_sig,delta,N_pts);
