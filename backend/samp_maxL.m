%Alex Tye
%Dec 2016
function [samples, logLk] = samp_maxL(N_coefs, samples, x_min, x_max, p_sig, delta, N_pts, filename)
...
%filename is the path and filename of the desired log file.
    ...
    logLk = zeros(1,size(samples,2));
    
    for i = 1:size(samples,2)
        obs_data = [samples(:,i) zeros(size(samples,1),1)];
        [best_coefs, bestLk, hessian, pchain, logLk(i), accRatio] = pchain_gen(obs_data, knots, priors_mu, cov_prior, N_pts, 0, fileID);
    end
